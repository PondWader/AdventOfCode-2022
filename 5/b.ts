...
for (const move of moves) {
    ...
    const crates = cratePositions[startStack].slice(0, amount);
    cratePositions[startStack] = cratePositions[startStack].slice(amount);
    cratePositions[finishStack].unshift(...crates);
}
